...
$.fn.rangeCalendar = function(options) {
		var currentDate = moment();//korashy
		var endDate = moment().add('days',6);//korashy
		// alert(currentDate);
    var defaults = {
			lang: "en",
			theme: "default-theme",
	...
	this.each(function(i, el) {
		...
		obj._initRangeBar = function(){
		    ...
		    //$(selectedCell).append('<div class="range-bar resizable"><div class="range-bar-content"></div></div>'); //duaa
		...
		obj._getCalendarHTML = function(startDate,endDate) {
			...
			//korasy function to get the size of supplied prices objet
			...
			var pricesObjectLength = countProperties(obj.prices);
			if(obj.type == 1){

				for (var index = 1; index<pricesObjectLength + 1; index++){
				
					var fullYear = date.format("YYYY");
					var endFullYear = endDate.format("YYYY");
					var month = date.format("MMM");
					var endMonth = endDate.format("MMM");
					var monthNumber = date.format("MM");
					var endMonthNumber = endDate.format("MM");
					var day = date.format('dd');
					var endDay = endDate.format('dd');
					var dayNumber = date.format('DD');
					var endDayNumber = endDate.format('DD');

                    cell = '<div class="cal-cell cell">';
                    cell += '<div class="cell-content"  date-id="'+fullYear+monthNumber+dayNumber+'" month-id="'+fullYear+''+monthNumber+'" month="'+monthNumber+'">';
                    cell += '<div class="date_price">';
                    cell += '<div class="day-number">'+dayNumber+'</div>';
                    cell+= '<div class="min_price"><span>'+ obj.prices[index] + '</span> <span>' +  obj.currency + '</span></div>';//korashy,duaa
                    cell += '</div>';//date_price
                    cell += '<div class="date"><span>'+ month +'</span><span class="day>">'+day+'</span></div>';
                    cell += '</div>';//end cell content
                    cell += '</div>';// cell
/*
                    cell += '<div class="cal-cell cell" >';
                    cell += '<div class="cell-content" date-id="'+endFullYear+endMonthNumber+endDayNumber+'" month-id="'+endFullYear+''+endMonthNumber+'" month="'+endMonthNumber+'">';
                    cell += '<div class="date_price">';
                    cell += '<div class="day-number">'+endDayNumber+'</div>';
                    cell+= '<div class="min_price"><span>'+ obj.prices[index] + '</span> <span>' +  obj.currency + '</span></div>';//korashy,duaa
                    cell += '</div>';//date_price
                    cell += '<div class="date"><span>'+ endMonth +'</span><span class="day> ">'+endDay+'</span></div>';
                    cell += '</div>';//end cell content
                    cell += '</div>';// cell
*/
						calendarHtml += cell;
				...
				for (var index = 1; index<pricesObjectLength + 1; index++){
                    ...
                    cell = '<div class="cal-cell cell return" >';
                    cell += '<div class="cell-content" date-id="'+fullYear+monthNumber+dayNumber+'" month-id="'+fullYear+''+monthNumber+'" month="'+monthNumber+'">';
						cell += '<div class="date_price">';
						cell += '<div class="day-number">'+dayNumber+'</div>';
						//cell+= '<div class="min_price"><span>'+ obj.prices[index] + '</span> <span>' +  obj.currency + '</span></div>';//korashy,duaa
		    ...
		    $el.hover(
				...
				function() {
				    ...
				    if(obj.autoHideMonths)
				    	obj.monthsObj.css('display','none');//korashy
				}, 
				function() {
		...
		obj._generateView = function () {
			...
			(obj.autoHideMonths ?  obj.monthsObj.removeClass("auto-hide-months") : '');//korashy
		...
		obj._init = function( element,options ) {
			...
			obj.prices = options.price;//korashy
			obj.currency = options.currency;//duaa
			obj.type = options.type;//duaa
